        ...
        def render_pass(view_angle, sh_id):
            ...
            for cfg, data in render_targets.items():
                ...
                light_image = Image.frombytes(
                    'RGBA', fbo2.size,
                    light_image_data).transpose(Image.Transpose.FLIP_TOP_BOTTOM)
                color_image_data = fbo2.read(components=4, attachment=1)
                color_image = Image.frombytes(
                    'RGBA', fbo2.size,
                    color_image_data).transpose(Image.Transpose.FLIP_TOP_BOTTOM)
                ...
                light_image.save(
                    os.path.join(
                        save_folder_tag,
                        f'{item}_{sh_id}_{view_angle}_light_color.png'))
                color_image.save(
                    os.path.join(save_folder_tag,
                                 f'{item}_{sh_id}_{view_angle}_color.png'))
                albedo_image.save(
                    os.path.join(save_folder_tag,
                                 f'{item}_{sh_id}_{view_angle}_albedo.png'))

        for view_angle in view_angles:
            for env_id in env_ids:
                render_pass(view_angle, env_id)
