...
class EncoderConfig:
    ...
    def __post_init__(self):
        if not isinstance(self.unet3d_kwargs, UNet3dConfig):
            self.unet3d_kwargs = UNet3dConfig(**self.unet3d_kwargs)
...
class ModelConfig:
    ...
    def __post_init__(self):
        if not isinstance(self.encoder_kwargs, EncoderConfig):
            self.encoder_kwargs = EncoderConfig(**self.encoder_kwargs)
...
class Config:
    ...
    def __post_init__(self):
        ...
        if not isinstance(self.test, TestConfig):
            self.test = TestConfig(**self.test)

        if not isinstance(self.random, RandomConfig):
            self.random = RandomConfig(**self.random)
