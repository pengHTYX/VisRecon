...
if __name__ == '__main__':
    ...
    for cubemap in cubemaps:
        ...
        def render_pass(angle, counter):
            ...
            light_image.save(
                os.path.join(save_folder, f"{model_name}_light.png"))
            color_image.save(
                os.path.join(save_folder, f"{model_name}_color.png"))

        for i in tqdm(range(frame_count)):
            render_pass(-math.radians(start_frame + i * interval), i)

        for image_tag in ['light', 'color']:
            cmd = f"gifski -H {height} -Q 100 -o {save_folder}/{image_tag}_{cubemap_name}.gif {save_folder}/*_{image_tag}.png"
            os.system(cmd)
