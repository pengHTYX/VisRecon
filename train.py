...
from realworld import THumanDataset as THumanRealWorldDataset
...
if __name__ == '__main__':
    ...
    if realworld:
        val_dataset = THumanRealWorldDataset(cfg.data.realworld_folder, cfg=cfg)
    else:
    ...
    if args.test:
        with torch.no_grad():
            for data_val in val_loader:
                if realworld:
                    frame = data_val['frame'][0].item()
                    model_name = f'realworld_{str(frame).zfill(4)}'
